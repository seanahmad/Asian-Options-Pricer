...
def mc_put_control_variates_gr(S0, K, T, t, r, q, sigma,npaths,timesteps):
  ...
  for i in range(1,npaths):
    ...
    S_arithm = np.mean(S)
    S_geom = np.exp(np.mean(np.log(S)))
    payoff_arithm = np.exp(-r * T) * max(K - np.mean(S_arithm), 0)
    payoff_geom = np.exp(-r * T) * max(K - np.mean(S_geom), 0)
    p_cv_temp.append(payoff_arithm - payoff_geom)
...
# Call Options:
mc_call_cv = mc_call_control_variates(S0, K, T, t, r, 0.0, sigma,1000,1000)
...
mc_plot_values(mc_call_control_variates_gr,S0, K, T, t, r, q, sigma, 10, 100, 'Call')
#mc_plot_values(mc_put_control_variates_gr,S0, K, T, t, r, q, sigma, 100, 1000, 'Put')

mc_plot_values(mc_call_geom,S0, K, T, t, r, q, sigma, 100, 1000 'Call')

mc_plot_values(mc_call_geom,S0, K, T, t, r, q, sigma, 100, 1000, 'Put')