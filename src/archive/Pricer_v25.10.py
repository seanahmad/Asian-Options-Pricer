...
def mc_call_cv(S0, K, T, t, r, q, sigma, n_paths, n_steps, n_sims):
  ...
  for i in range(1,n_sims):
    ...
    c_cv.append(jnp.mean(c_cv_temp))
    c_upper_cv.append(jnp.mean(c_cv) + 1.96 * np.std(c_cv)/np.sqrt(i))
    c_lower_cv.append(jnp.mean(c_cv) - 1.96 * np.std(c_cv)/np.sqrt(i))
...
def mc_put_cv(S0, K, T, t, r, q, sigma, n_paths, n_steps, n_sims):
  ...
  for i in range(1,n_sims):
    ...
    p_cv.append(jnp.mean(p_cv_temp))
...
mc_plot_values(mc_call_geom,S0, K, T, t, r, q, sigma,100000,1000,1000, "Call")
...
mc_plot_values(mc_call_geom,100, 95, 1.0, 0.0, 0.15, 0.0, 0.3,1000,1000,'Call')
...
class LiveOptionsGraph:
    ...
    def time_step(self, z):
        ...
        self.axs[2].cla()
        #self.axs[3].cla()
        self.axs[0].plot(self.index_set, self.option_prices, label='BSM Price', c='b')
        ...
        if self.type == 'call':
            if self.strike_price <= self.asset_prices[self.index]:
                self.axs[2].plot(self.index_set, self.asset_prices + price_step, animated=True, label='Asset Price', c='g')
                self.axs[2].axhline(y=self.strike_price, label='Call Strike', c='gray')
                ...
                self.axs[2].plot(self.index_set, self.asset_prices, animated=True, label='Asset Price', c='r')
                self.axs[2].axhline(y=self.strike_price, label='Call Strike', c='gray')
        elif self.type == 'put':
            if self.strike_price < self.asset_prices[self.index]:
                self.axs[2].plot(self.index_set, self.asset_prices, animated=True, label='Asset Price', c='r')
                self.axs[2].axhline(y=self.strike_price, label='Put Strike', c='gray')
            else:
                self.axs[2].plot(self.index_set, self.asset_prices, animated=True, label='Asset Price', c='g')
                self.axs[2].axhline(y=self.strike_price, label='Put Strike', c='gray')
    ...
    def __init__(self, asian_option, type):
        ...
        self.option_prices = []
        self.hedged = []
        self.asset_prices = [asian_option.asset_price]
        self.deltas = []
...
initial_ac = AsianCall(100, 95, 0.3, datetime.date(2021, 4, 29), 0.15, .5)
initial_ap = AsianPut(100, 95, 0.3, datetime.date(2021, 4, 29), 0.15, .5)
