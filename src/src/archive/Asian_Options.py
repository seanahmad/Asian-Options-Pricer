...
def difference(iterations=10000000, K=90, T=1, r=0.1, N=150, S0=100, sigma=0.3):
    ...
    geometric_sampling = np.exp(-r * T) * np.maximum(
        S0 * np.exp(sigma * np.mean(brownian, axis=0) + (r - (sigma ** 2) / 2) * (T / 2)) - K, 0)
...
def Monte_Carlo_Arithmetic(iterations=10000, K=110, T=1, r=0.1, N=150, S0=100, sigma=0.3):
    # Generate N*iterations gaussian samples of variance T/N
    times = (T / N) * np.arange(1, N + 1)
    increase = np.random.normal(0, np.sqrt(T / N), (N, iterations))
    brownian = np.dot(np.tril(np.ones((N, N))), increase)
...
data_lognormal = Monte_Carlo_lognormal_it(iterations=1000000, K=110, T=1, r=0.1)
...
Monte_Carlo_diff(iterations=1000000, K=110, T=1, r=0.1, N=150, S0=100, sigma=0.2)
